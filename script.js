...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function calculateTripBudget() {
    const { days, people, style, vehicle, scope } = window.calcState;
    const nights = days - 1;
    
    // 1. Stay cost
    let stayCostPerNight;
    if (people === 1) {
      stayCostPerNight = (style === 'homestay') ? 350000 : 1500000;
    } else {
      stayCostPerNight = (style === 'homestay') ? 600000 : 2200000;
    }
    let roomCount = (people === 1) ? 1 : Math.ceil(people / 2);
    let totalStayCost = stayCostPerNight * nights * (people === 1 ? 1 : roomCount);
    ...
    if (scope === 'full') {
      ...
      let tourCostPerPerson = 650000; // Kỳ Co - Eo Gió Cano + Lặn San Hô
      if (days >= 3) tourCostPerPerson += 250000; // Tây Sơn Hầm Hô Tour
      totalTourCost = tourCostPerPerson * people;
      
      // 3. Food cost
      let foodCostPerPersonPerDay = 300000;
      totalFoodCost = foodCostPerPersonPerDay * days * people;
      
      // 4. Vehicle cost
      let vehicleCostPerDay = (vehicle === 'motorbike') ? ((people === 1) ? 120000 : 150000) : 800000;
      let vehicleCount = (vehicle === 'motorbike') ? Math.ceil(people / 2) : 1;
      totalVehicleCost = vehicleCostPerDay * days * vehicleCount;
    }
    ...
    if (scope === 'room_only') {
      if (perPersonElem) perPersonElem.innerHTML = `${fmt(perPerson)} <span style="font-size:14px; font-weight:600; color:#cbd5e1;">/ người (tiền phòng)</span>`;
      if (totalElem) totalElem.innerText = `👉 Tổng tiền phòng nghỉ (${nights} đêm, ${people} người): ${fmt(grandTotal)}`;
      
      if (breakdownStay) breakdownStay.innerText = `🏨 Tiền lưu trú phòng nghỉ (${nights} đêm, ${roomCount} phòng): ~${fmt(totalStayCost)}`;
      if (breakdownTour) breakdownTour.innerText = `🚤 Tour Cano & Vé tham quan: Đã ẩn (Khách tự túc)`;
      if (breakdownFood) breakdownFood.innerText = `🍲 Ăn uống hải sản: Đã ẩn (Khách tự túc)`;
      if (breakdownVehicle) breakdownVehicle.innerText = `🛵 Thuê xe di chuyển: Đã ẩn (Khách tự túc)`;
    } else {
      if (perPersonElem) perPersonElem.innerHTML = `${fmt(perPerson)} <span style="font-size:14px; font-weight:600; color:#cbd5e1;">/ người</span>`;
      
      if (people === 1) {
        if (totalElem) totalElem.innerText = `👉 Tổng chuyến đi trọn gói (1 người solo): ${fmt(grandTotal)}`;
        if (breakdownStay) breakdownStay.innerText = `🏨 Tiền lưu trú (${nights} đêm phòng đơn/Dorm): ~${fmt(totalStayCost)}`;
      } else {
        if (totalElem) totalElem.innerText = `👉 Tổng chuyến đi (${people} người, ${days}N${nights}Đ): ${fmt(grandTotal)}`;
        if (breakdownStay) breakdownStay.innerText = `🏨 Tiền lưu trú (${nights} đêm, ${roomCount} phòng đôi): ~${fmt(totalStayCost)}`;
      }
